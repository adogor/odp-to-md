...
function OdpConverter(filepath, outDir) {
    ...
    this.name = path.basename(this.filePath, '.odp').toLowerCase().replace(/[\s-]+/g, '_').replace(/_{2,}/g, "_");
    ...
    this.convert = function() {
        ...
        var readStream = fs.createReadStream(this.contentName).pipe(saxStream);

        readStream.on("end", function() {
            fs.unlink(this.contentName);
        }.bind(this));
    }
}
